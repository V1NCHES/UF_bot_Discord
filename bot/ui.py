...
def generate_combined_call_message(call_data, include_ping=True):
    ...
    if (call_data.get('call_type') in ['zvz', 'roum', 'call'] or call_data.get('slot_limits')) and call_data.get('slot_limits'):
        for i, slot in enumerate(call_data['slots']):
            label = call_data['slot_labels'][i]
            limit = call_data['slot_limits'][i]
            if slot:
                mentions_str = ", ".join([u.mention for u in slot])
                content += f"**{i+1}.** {label} — ({len(slot)} из {limit}) {mentions_str}\n"
            else:
                content += f"**{i+1}.** {label} — (0 из {limit})\n"
    else:
...
async def auto_finish_call(client, call_data, active_calls):
    ...
    for slot in call_data['slots']:
        if isinstance(slot, list):
            ...
            summary += f"{slot.id}\t{slot.display_name}\n"
            has_participants = True
    ...
    try:
        from bot.cogs.voice_cog import generate_voice_finish_report
        guild = client.get_guild(call_data.get('guild_id'))
        if guild:
            voice_embed = generate_voice_finish_report(guild, call_data)
            target_chan = client.get_channel(thread_id) if thread_id else client.get_channel(main_channel_id)
            if target_chan and voice_embed:
                await target_chan.send(embed=voice_embed)
    except Exception as err:
        print(f"Ошибка отправки voice_embed в auto_finish_call: {err}")
...
class RoleButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        status_msg = ""
        if (call_data.get('call_type') in ['zvz', 'roum', 'call'] or call_data.get('slot_limits')) and call_data.get('slot_limits'):
            slot_list = call_data['slots'][self.slot_idx]
            limit = call_data['slot_limits'][self.slot_idx]
            
            if interaction.user in slot_list:
                slot_list.remove(interaction.user)
                status_msg = f"❌ {interaction.user.display_name} выписался из слота **{call_data['slot_labels'][self.slot_idx]}**"
            else:
                user_registered_in_other = False
                for other_slot in call_data['slots']:
                    if isinstance(other_slot, list) and interaction.user in other_slot:
                        user_registered_in_other = True
                        break
                if user_registered_in_other:
                ...
                if len(slot_list) >= limit:
                    return await send_to_thread(f"⚠️ {interaction.user.mention}, эта роль уже полностью заполнена ({limit} из {limit})!")
                
                slot_list.append(interaction.user)
...
class SaveTemplateModal(discord.ui.Modal, title="Сохранение шаблона сбора"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            from bot.utils import save_call_to_cache
        ...
        if interaction.message.id in self.active_calls:
            ...
            try:
                ...
                if interaction.guild:
                    v_embed = generate_voice_finish_report(interaction.guild, call_data)
                    if v_embed:
                        target_ch = interaction.message.thread or interaction.channel
                        await target_ch.send(embed=v_embed)
            except Exception as e:
                print(f"Ошибка отправки voice finish report: {e}")
...
class AddSlotModal(discord.ui.Modal, title="Добавление новой роли"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if self.call_data.get('call_type') in ['zvz', 'roum', 'call'] or self.call_data.get('slot_limits'):
            new_roles = parse_multi_roles_text(self.label_input.value)
            if not new_roles:
                return await interaction.response.send_message("❌ Неверный формат ролей!", ephemeral=True)
            
            total_slots = len(self.call_data['slots']) + len(new_roles)
            if total_slots > 21:
                allowed_to_add = 21 - len(self.call_data['slots'])
                if allowed_to_add <= 0:
                    return await interaction.response.send_message("❌ Достигнут лимит в 21 роль для одного сбора!", ephemeral=True)
                new_roles = new_roles[:allowed_to_add]
                
            added_labels = []
            for label, limit in new_roles:
                self.call_data['slot_labels'].append(label)
                self.call_data['slots'].append([])
                if 'slot_limits' not in self.call_data or self.call_data['slot_limits'] is None:
                    self.call_data['slot_limits'] = []
                self.call_data['slot_limits'].append(limit)
                added_labels.append(f"{label} ({limit})")
            added_text = ", ".join(added_labels)
        else:
            new_labels = parse_roles_text(self.label_input.value)
            if not new_labels:
                return await interaction.response.send_message("❌ Неверный формат ролей!", ephemeral=True)

            total_slots = len(self.call_data['slots']) + len(new_labels)
            if total_slots > 21:
                allowed_to_add = 21 - len(self.call_data['slots'])
                if allowed_to_add <= 0:
                    return await interaction.response.send_message("❌ Достигнут лимит в 21 роль для одного сбора! Нельзя добавить новые роли.", ephemeral=True)
                new_labels = new_labels[:allowed_to_add]
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if 0 <= idx < len(self.call_data['slots']):
                ...
                if 'slot_limits' in self.call_data and self.call_data['slot_limits'] is not None and len(self.call_data['slot_limits']) > idx:
                    self.call_data['slot_limits'].pop(idx)
...
def parse_multi_roles_text(roles_text: str) -> list[tuple[str, int]]:
    import re
    roles = []
    for line in roles_text.split('\n'):
        ...
        # Ищем разделители типа ; : - пробел x/х и лимит в конце, e.g. "tank;5", "tank 5", "tank x5", "саппорт 3"
        match = re.search(r"(?:[;:\-–—\s]+|[;\s]*[xх]\s*)\s*(\d+)\s*(?:шт\.?|pcs)?$", clean_label, re.IGNORECASE)
        if match:
            role_name = clean_label[:match.start()].strip()
            if not role_name:
                role_name = clean_label
                limit = 1
            else:
                try:
        ...
        roles.append((role_name, limit))
    return roles

parse_zvz_roles_text = parse_multi_roles_text

def parse_roles_text(roles_text: str) -> list[str]:
    import re
    labels = []
    for line in roles_text.split('\n'):
        line = line.strip()
        ...
        # Поиск количества в конце строки, например "Танк 5", "Танк x5", "Танк х5", "Танк - 5", "Танк 5шт"
        match = re.search(r"\s+([xх\-–—]|шт\.?)?\s*(\d+)\s*(?:шт\.?|pcs)?$", clean_label, re.IGNORECASE)
        if match:
            role_name = clean_label[:match.start()].strip()
...
class CreateCallModal(discord.ui.Modal, title="Создание нового сбора"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        from bot import logger
        import asyncio
        from bot.utils import parse_msk_time
        ...
        # Парсим роли из текстового поля с учетом количества
        if self.call_type in ['zvz', 'roum', 'call']:
            multi_roles = parse_multi_roles_text(self.roles_input.value)
            if not multi_roles:
                multi_roles = [("Участник", 1)]
            if len(multi_roles) > 21:
                multi_roles = multi_roles[:21]
            labels = [r[0] for r in multi_roles]
            limits = [r[1] for r in multi_roles]
            slots = [[] for _ in labels]
        else:
            labels = parse_roles_text(self.roles_input.value)
            if not labels:
                labels = ["Участник"]
            if len(labels) > 21:
                labels = labels[:21]
...
class AssignPlayerModal(discord.ui.Modal, title="Добавление игрока на роль"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            for other_slot in self.call_data['slots']:
                if isinstance(other_slot, list):
                    if member in other_slot:
                        is_already_registered = True
                        break
                elif other_slot == member:
                    is_already_registered = True
                    break
            ...
            # Записываем
            if (self.call_data.get('call_type') in ['zvz', 'roum', 'call'] or self.call_data.get('slot_limits')) and 'slot_limits' in self.call_data:
                slot_list = self.call_data['slots'][idx]
                limit = self.call_data['slot_limits'][idx]
                if len(slot_list) >= limit:
                    return await interaction.response.send_message(f"⚠️ Роль уже полностью заполнена ({limit} из {limit})!", ephemeral=True)
...
class OrganizerMenuView(discord.ui.View):
    ...
    async def assign_player(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "💡 **Чтобы добавить игрока с удобным поиском и пингом (@упоминанием):**\n"
            "Используйте команду в строке ввода Discord:\n"
            "> **`/add_player role_number: [№ роли] player: @Игрок`**\n\n"
            "Либо напишите прямо в ветке сбора:\n"
            "> **`!+[№ роли] @Игрок`** (например: `!+1 @Nickname`)",
            ephemeral=True
        )
    ...
    async def summon_all(self, interaction: discord.Interaction, button: discord.ui.Button):
        participants = []
        for slot in self.call_data['slots']:
            if isinstance(slot, list):
                for user in slot:
...
class CallRoleView(discord.ui.View):
    ...
    def __init__(self, call_data, active_calls):
        ...
        for i, label in enumerate(call_data['slot_labels'][:slots_limit]):
            ...
            if (call_data.get('call_type') in ['zvz', 'roum', 'call'] or call_data.get('slot_limits')) and isinstance(call_data['slots'][i], list):
                if call_data['slots'][i]:
