...
async def save_data_async(call_data):
    """Асинхронное сохранение в Google Таблицы"""
    await asyncio.to_thread(save_to_google_sheets, call_data)
...
def save_to_google_sheets(call_data):
    """Сохранение данных в Google Таблицу в формате 2-х колонок"""
    if not os.path.exists(config.CREDENTIALS_FILE):
        return
    ...
    try:
        ...
        if target_col_idx == -1:
            date_str = datetime.now().strftime("%Y-%m-%d %H:%M")
            if not all_values or not all_values[0] or not all_values[0][0]:
                target_col_idx = 0
            else:
                sheet.insert_cols([[""] * (len(call_data['slots']) + 20)] * 3, col=1)
                target_col_idx = 0
        ...
        end_a1 = gspread.utils.rowcol_to_a1(max(len(rows) + 20, len(all_values) if all_values else 0), target_col_idx + 3)
        
        empty_block = [["", "", ""]] * (max(len(rows) + 20, len(all_values) if all_values else 0))
        sheet.update(range_name=f"{start_a1}:{end_a1}", values=empty_block)
        
        actual_end_a1 = gspread.utils.rowcol_to_a1(len(rows), target_col_idx + 3)
        sheet.update(range_name=f"{start_a1}:{actual_end_a1}", values=rows)
...
def clear_google_sheets_data(call_data):
    ...
    try:
        ...
        if target_col_idx != -1:
            start_a1 = gspread.utils.rowcol_to_a1(1, target_col_idx + 1)
            end_a1 = gspread.utils.rowcol_to_a1(len(all_values), target_col_idx + 3)
            empty_block = [["", "", ""]] * len(all_values)
            sheet.update(range_name=f"{start_a1}:{end_a1}", values=empty_block)
    except Exception as e:
